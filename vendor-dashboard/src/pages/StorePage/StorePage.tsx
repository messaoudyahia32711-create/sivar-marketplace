...
import {
  ...
  MapPin, CheckCircle, Package, Briefcase, Store, X, Clock, Tag
} from 'lucide-react'
        ...
        >
          ...
          <div style={{ position: 'relative', width: '100%', aspectRatio: '4/3', background: '#0a0a0a', borderRadius: '24px 24px 0 0', overflow: 'hidden' }}>
            ...
            <button
              ...
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(212,175,55,0.8)'; e.currentTarget.style.color = '#0F0F0F' }}
            ...
            {badgeLabel && (
              <div style={{
                ...
                padding: '5px 16px', borderRadius: 20, fontSize: '0.85rem',
                border: '1px solid #D4AF37', backdropFilter: 'blur(5px)', fontWeight: 700,
              }}>
                {badgeLabel}
              </div>
            )}
            ...
            {images.length > 1 && (
              <>
                <button
                  ...
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(212,175,55,0.7)' }}
              ...
              <div style={{
                ...
                {images.map((_, idx) => (
                  <button
                    ...
                    style={{
                      width: idx === currentImg ? 24 : 8, height: 8,
                      borderRadius: 4, border: 'none', cursor: 'pointer',
                      background: idx === currentImg ? '#D4AF37' : 'rgba(255,255,255,0.4)',
                      transition: 'all 0.3s',
                    }}
                  />
                ))}
              ...
              {images.map((img, idx) => (
                <button
                  ...
                  style={{
                    ...
                    border: idx === currentImg ? '2px solid #D4AF37' : '2px solid rgba(255,255,255,0.1)',
                    opacity: idx === currentImg ? 1 : 0.5,
                    transition: 'all 0.3s', cursor: 'pointer',
                    boxShadow: idx === currentImg ? '0 0 12px rgba(212,175,55,0.3)' : 'none',
                  }}
                >
          ...
          <div style={{ padding: '20px 24px 28px' }}>
            ...
            {d.avg_rating && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
                {[1,2,3,4,5].map(s => (
                  <Star key={s} style={{
                    width: 14, height: 14,
                    fill: s <= Math.round(d.avg_rating!) ? '#D4AF37' : 'none',
                    color: '#D4AF37',
                  }} />
                ))}
                <span style={{ color: '#888', fontSize: '0.85rem', marginRight: 4 }}>
                  ({d.avg_rating?.toFixed(1)})
                </span>
                {isProduct && (d as Product).reviews_count > 0 && (
                  <span style={{ color: '#666', fontSize: '0.8rem' }}>
                    · {(d as Product).reviews_count} تقييم
                  </span>
                )}
              </div>
            )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              <span style={{ fontSize: '1.6rem', color: '#D2691E', fontWeight: 900 }}>
                {fmt(d.price)}
              </span>
              {isProduct && (d as Product).discount_price && (
                <>
                  <span style={{
                    ...
                    {fmt((d as Product).discount_price!)}
                  </span>
                  <span style={{
            ...
            <div style={{
              ...
              <h4 style={{ color: '#D4AF37', fontSize: '0.9rem', fontWeight: 700, marginBottom: 8 }}>
                الوصف
              </h4>
              <p style={{ color: '#aaa', fontSize: '0.9rem', lineHeight: 1.9 }}>
                {d.description || 'لا يوجد وصف متوفر لهذا المنتج.'}
              </p>
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 24 }}>
              {isProduct && (
                <>
                  <div style={{
                    ...
                    <div>
                      <div style={{ fontSize: '0.7rem', color: '#888' }}>الفئة</div>
                      <div style={{ fontSize: '0.85rem', color: '#D4AF37', fontWeight: 700 }}>
                        {(d as Product).category_name || 'عام'}
                      </div>
                    </div>
                  </div>
                  <div style={{
                    background: (d as Product).stock_status === 'out' ? 'rgba(239,68,68,0.08)' : 'rgba(16,185,129,0.08)',
                    border: `1px solid ${(d as Product).stock_status === 'out' ? 'rgba(239,68,68,0.2)' : 'rgba(16,185,129,0.2)'}`,
                    borderRadius: 12, padding: '8px 14px',
                    display: 'flex', alignItems: 'center', gap: 8,
                  }}>
                    <Package style={{ width: 14, height: 14, color: (d as Product).stock_status === 'out' ? '#ef4444' : '#10b981' }} />
                    <div>
                      <div style={{ fontSize: '0.7rem', color: '#888' }}>المخزون</div>
                      <div style={{
                        fontSize: '0.85rem', fontWeight: 700,
                        color: (d as Product).stock_status === 'out' ? '#ef4444' : '#10b981',
                      }}>
                        {(d as Product).stock_status === 'out' ? 'نفذ' : (d as Product).stock_status === 'critical' ? 'ينفذ قريباً' : 'متوفر'}
                      </div>
                    </div>
                  </div>
                </>
              ...
              {!isProduct && (d as Service).duration_hours && (
                <div style={{
                  ...
                  <div>
                    <div style={{ fontSize: '0.7rem', color: '#888' }}>المدة</div>
                    <div style={{ fontSize: '0.85rem', color: '#D2691E', fontWeight: 700 }}>
                      {(d as Service).duration_hours} ساعة
                    </div>
                  </div>
                </div>
              )}
            ...
            <button
              ...
              style={{
                ...
                background: isOut ? '#333' : 'linear-gradient(135deg, #D4AF37, #c9a22e)',
                color: isOut ? '#666' : '#0F0F0F',
                border: 'none', borderRadius: 14,
                fontFamily: 'Tajawal, sans-serif', fontWeight: 900, fontSize: '1.05rem',
                cursor: isOut ? 'not-allowed' : 'pointer',
                transition: 'transform 0.2s, box-shadow 0.2s',
                boxShadow: isOut ? 'none' : '0 8px 24px rgba(212,175,55,0.3)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
              }}
              onMouseEnter={e => { if (!isOut) e.currentTarget.style.transform = 'translateY(-2px)' }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)' }}
            >
              {isProduct ? (
                <><ShoppingCart style={{ width: 18, height: 18 }} />{isOut ? 'غير متوفر حالياً' : 'أضف للسلة'}</>
              ) : (
                <><Briefcase style={{ width: 18, height: 18 }} />اطلب الخدمة</>
              )}
            </button>
          </div>
        </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: hovered ? 'translateY(-10px) rotateX(3deg)' : 'translateY(0)',
        boxShadow: hovered ? '0 20px 40px rgba(212, 175, 55, 0.2)' : '8px 8px 20px rgba(0,0,0,0.6)',
        border: '1px solid rgba(255,255,255,0.05)',
        cursor: 'pointer',
      }}
    >
      <div style={{ width: '100%', height: '200px', position: 'relative', overflow: 'hidden', background: '#222' }}>
        <img
          src={product.image_main || IMG_PLACEHOLDER} alt={product.name}
          style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.5s', transform: hovered ? 'scale(1.1)' : 'scale(1)' }}
          onError={(e) => { (e.target as HTMLImageElement).src = IMG_PLACEHOLDER }}
        />
        {badgeLabel && (
          <div style={{ position: 'absolute', top: 15, right: 15, background: 'rgba(0,0,0,0.7)', color: 'var(--gold)', padding: '5px 15px', borderRadius: '20px', fontSize: '0.8rem', border: '1px solid var(--gold)', backdropFilter: 'blur(5px)' }}>
            {badgeLabel}
          </div>
        )}
        {/* Image count indicator */}
        {product.images && product.images.length > 0 && (
          <div style={{ position: 'absolute', bottom: 10, left: 10, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', padding: '3px 8px', borderRadius: 8, color: '#ccc', fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: 4 }}>
            📷 {product.images.length + (product.image_main ? 1 : 0)}
          </div>
        )}
        {isOut && (
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <span style={{ color: '#ef4444', fontWeight: 900, fontSize: '1.1rem' }}>نفذ المخزون</span>
          </div>
        )}
      </div>
      <div style={{ padding: '20px' }}>
        {product.avg_rating && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 8 }}>
            {[1,2,3,4,5].map(s => (
              <Star key={s} style={{ width: 12, height: 12, fill: s <= Math.round(product.avg_rating!) ? 'var(--gold)' : 'none', color: 'var(--gold)' }} />
            ))}
            <span style={{ fontSize: '0.75rem', color: 'var(--text-gray)', marginRight: 4 }}>({product.avg_rating?.toFixed(1)})</span>
          </div>
        )}
        ...
        <div style={{ marginBottom: 15 }}>
          ...
          {product.discount_price && (
            <span style={{ fontSize: '0.85rem', color: 'var(--text-gray)', textDecoration: 'line-through', marginRight: 8 }}>{fmt(product.discount_price)}</span>
          )}
        </div>
        <button
          disabled={isOut}
          ...
          style={{
            ...
            border: '2px solid var(--gold)', borderRadius: '10px',
            fontFamily: 'Tajawal, sans-serif', fontWeight: 'bold',
            cursor: isOut ? 'not-allowed' : 'pointer', transition: 'all 0.3s', opacity: isOut ? 0.5 : 1,
          }}
          onMouseEnter={(e) => { if (!isOut) { e.currentTarget.style.background = 'var(--gold)'; e.currentTarget.style.color = '#0F0F0F' } }}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hovered ? '0 20px 40px rgba(210, 105, 30, 0.2)' : '8px 8px 20px rgba(0,0,0,0.6)',
        border: '1px solid rgba(255,255,255,0.05)',
        cursor: 'pointer',
      }}
    >
      <div style={{ width: '100%', height: '200px', overflow: 'hidden', background: '#222', position: 'relative' }}>
        ...
        {service.images && service.images.length > 0 && (
          <div style={{ position: 'absolute', bottom: 10, left: 10, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', padding: '3px 8px', borderRadius: 8, color: '#ccc', fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: 4 }}>
            📷 {service.images.length + (service.image_main ? 1 : 0)}
          </div>
        )}
      </div>
      <div style={{ padding: '20px' }}>
        {service.category_name && (
          <span style={{ display: 'inline-block', fontSize: '0.75rem', color: 'var(--sand-light)', border: '1px solid rgba(210,105,30,0.4)', borderRadius: '20px', padding: '2px 10px', marginBottom: 8 }}>
            {service.category_name}
          </span>
        )}
        {service.avg_rating && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
            {[1,2,3,4,5].map(s => (
              <Star key={s} style={{ width: 12, height: 12, fill: s <= Math.round(service.avg_rating!) ? 'var(--gold)' : 'none', color: 'var(--gold)' }} />
            ))}
          </div>
        )}
        ...
        <button
          ...
          onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--gold)'; e.currentTarget.style.color = '#0F0F0F' }}
          onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--gold)' }}
        >
          <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
...
export default function StorePage() {
  ...
  const handleMessage = async () => {
    ...
    try {
      ...
      if (!token) {
        alert('يجب تسجيل الدخول كزبون للمراسلة')
        window.location.href = '/customer-dashboard/login'
        return
      }
      await axios.post(`${API_BASE}/api/chat/messages/send/`, 
        { content: 'مرحباً، أود الاستفسار حول متجركم.', receiver_id: data.vendor.id },
        { headers: { Authorization: `Bearer ${token}` } }
      )
      window.location.href = '/customer-dashboard/messages'
    } catch (err) {
      alert('حدث خطأ أثناء محاولة بدء المراسلة')
    } finally {
  ...
  const storeStyles = `
    ...
    .store-page-body {
      ...
      background-image:
        ...
        radial-gradient(circle at 90% 80%, rgba(210, 105, 30, 0.05) 0%, transparent 20%);
      min-height: 100vh;
      direction: rtl;
    }
  ...
  if (error) {
    return (
      <div style={{ minHeight: '100vh', background: '#0F0F0F', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16, direction: 'rtl', fontFamily: 'Tajawal, sans-serif' }}>
        <Store style={{ width: 64, height: 64, color: '#D4AF37', opacity: 0.4 }} />
        <h2 style={{ color: '#fff', fontSize: '1.5rem' }}>المتجر غير موجود</h2>
        <p style={{ color: '#888' }}>لم يتم العثور على هذا المتجر</p>
      ...
      <div className="store-page-body" style={{ position: 'relative' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, height: 54, background: 'rgba(15, 15, 15, 0.9)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', zIndex: 1000, borderBottom: '1px solid rgba(212, 175, 55, 0.1)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Store style={{ width: 16, height: 16, color: '#D4AF37' }} />
            <span style={{ color: '#D4AF37', fontWeight: 700, fontSize: '0.9rem', fontFamily: 'Tajawal, sans-serif' }}>{store?.name || vendor?.username || 'المتجر'}</span>
          </div>
          <div style={{ width: 40 }} />
        </div>

        {/* ── Contact Buttons ── */}
        <div style={{ position: 'fixed', bottom: 30, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 10, zIndex: 1000, animation: 'float-btn 3s ease-in-out infinite' }}>
          <button
            style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 24px', background: 'linear-gradient(135deg, #D4AF37, #c9a22e)', color: '#0F0F0F', border: 'none', borderRadius: '50px', fontFamily: 'Tajawal, sans-serif', fontSize: '1.05rem', fontWeight: 900, cursor: 'pointer', boxShadow: showContactPulse ? '0 8px 30px rgba(212, 175, 55, 0.5), 0 0 0 8px rgba(212, 175, 55, 0.1)' : '0 8px 30px rgba(212, 175, 55, 0.35)', transition: 'box-shadow 0.4s' }}
            onClick={() => { const phone = vendor?.phone || store?.phone; if (phone) window.open(`tel:${phone}`) }}
          >
            تواصل هاتفياً
          </button>
          <button
            style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 24px', background: '#222', color: '#D4AF37', border: '1px solid #D4AF37', borderRadius: '50px', fontFamily: 'Tajawal, sans-serif', fontSize: '1.05rem', fontWeight: 900, cursor: isMessaging ? 'wait' : 'pointer' }}
            onClick={handleMessage}
            disabled={isMessaging}
          >
            <MessageCircle style={{ width: 22, height: 22 }} />
            {isMessaging ? 'جاري التحويل...' : 'مراسلة'}
          </button>
        </div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', paddingBottom: 120, paddingTop: 54, position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ position: 'relative', height: 380, borderRadius: '0 0 30px 30px', overflow: 'hidden', boxShadow: '0 15px 30px rgba(0,0,0,0.5)', marginBottom: 40 }}>
            {isLoading ? (
              <div className="skeleton-animate" style={{ width: '100%', height: '100%' }} />
            ) : (
            ...
            <div style={{ position: 'absolute', bottom: 25, right: 30, zIndex: 10 }}>
              <div style={{ background: 'linear-gradient(135deg, #0F0F0F, #1A1A1A)', padding: '15px 25px', borderRadius: 15, border: '1px solid rgba(212, 175, 55, 0.3)', boxShadow: '0 10px 20px rgba(0,0,0,0.3)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <h1 style={{ fontSize: '1.8rem', fontWeight: 900, background: 'linear-gradient(to right, #D4AF37, #F7E7CE)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', letterSpacing: 1, fontFamily: 'Tajawal, sans-serif' }}>
                    {isLoading ? '...' : (store?.name || `متجر ${vendor?.username}`)}
                  </h1>
                  {(vendor?.is_verified || store?.is_verified) && <CheckCircle style={{ width: 20, height: 20, color: '#D4AF37', flexShrink: 0 }} />}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ color: '#D2691E', fontSize: '0.9rem', fontWeight: 700 }}>تاجر معتمد في Sivar</span>
            ...
            <div style={{ position: 'absolute', top: 18, left: 20, display: 'flex', gap: 12 }}>
              <div style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', padding: '6px 14px', borderRadius: 20, border: '1px solid rgba(212,175,55,0.2)', color: '#fff', fontSize: '0.8rem', fontFamily: 'Tajawal, sans-serif', display: 'flex', alignItems: 'center', gap: 6 }}>
                <Package style={{ width: 14, height: 14, color: '#D4AF37' }} />
                <span>{isLoading ? '...' : products.length} منتج</span>
              </div>
              <div style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', padding: '6px 14px', borderRadius: 20, border: '1px solid rgba(210,105,30,0.2)', color: '#fff', fontSize: '0.8rem', fontFamily: 'Tajawal, sans-serif', display: 'flex', alignItems: 'center', gap: 6 }}>
                <Briefcase style={{ width: 14, height: 14, color: '#D2691E' }} />
                <span>{isLoading ? '...' : services.length} خدمة</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 20, marginBottom: 40, perspective: 1000 }}>
            {(['products', 'services'] as const).map((tab) => (
              ...
              >
                {tab === 'products' ? (
                  <><Package style={{ width: 18, height: 18 }} /> المنتجات{' '}{!isLoading && <span style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 20, padding: '1px 8px', fontSize: '0.85rem' }}>{products.length}</span>}</>
                ) : (
                  <><Briefcase style={{ width: 18, height: 18 }} /> الخدمات{' '}{!isLoading && <span style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 20, padding: '1px 8px', fontSize: '0.85rem' }}>{services.length}</span>}</>
                )}
              </button>
            ))}
          </div>
                ...
                : products.length === 0
                  ...
                  : products.map((p, i) => (
                    <div key={p.id} className="animate-fade-in-up" style={{ animationDelay: `${i * 0.08}s` }}>
                      <ProductCard product={p} onClick={() => setModalItem({ type: 'product', data: p })} />
                    </div>
                  ))
              }
            </div>
                ...
                : services.length === 0
                  ...
                  : services.map((s, i) => (
                    <div key={s.id} className="animate-fade-in-up" style={{ animationDelay: `${i * 0.08}s` }}>
                      <ServiceCard service={s} onClick={() => setModalItem({ type: 'service', data: s })} />
                    </div>
                  ))
              }
            </div>
