...
import { Store, Camera, Save, Phone, FileText, Shield } from 'lucide-react'
...
export default function SettingsPage() {
  const [formData, setFormData] = useState<any>({
    name: '', description: '', phone: '', wilaya_id: '', return_policy: '',
  })
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in" dir="rtl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-3xl font-black">إعدادات المتجر</h2>
          <p className="text-sm text-muted-foreground">خصّص متجرك وعلامتك التجارية</p>
        </div>
      </div>
          ...
          <div className="absolute bottom-4 right-8">
            <div className="w-20 h-20 rounded-2xl border-4 border-background bg-background shadow-xl relative overflow-hidden group/logo">
              {previews.logo ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-white/5"><Store className="w-6 h-6 text-muted-foreground opacity-30" /></div>
              )}
