...
import { 
  ...
  Bell, Sun, Moon, Search, Mail, GraduationCap, ClipboardList,
  UserCheck
} from 'lucide-react'
...
const menuItems = [
  ...
  { icon: Building2,       label: 'إدارة المؤسسات',  path: '/organizations',  color: 'text-violet-400' },
  { icon: ClipboardList,   label: 'طلبات الانضمام',  path: '/join-requests', color: 'text-amber-400' },
  { icon: UserCheck,      label: 'أعضاء الفريق',    path: '/team',          color: 'text-emerald-400' },
  { icon: Mail,            label: 'الرسائل العلمية', path: '/messages',      color: 'text-indigo-400' },
  { icon: BarChart3,       label: 'إحصائيات الأداء', path: '/analytics',     color: 'text-cyan-400' },
  { icon: Settings,        label: 'إعدادات الحاضنة', path: '/settings',      color: 'text-gray-400' },
]
    ...
    <div className="min-h-screen bg-background flex bg-grid" dir="rtl">
      ...
      <aside className="hidden lg:flex flex-col w-72 glass-sidebar sticky top-0 h-screen z-50">
        ...
        <div className="p-6 border-b border-white/5 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold">لوحة الحاضنة</h1>
            <p className="text-[10px] text-muted-foreground">{user?.university_name || 'Algerian University'}</p>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-white/5 space-y-3">
          <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-white/5">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-[10px] text-muted-foreground">مدير حاضنة جامعية</p>
            </div>
          </div>
          <Button 
            variant="ghost" 
      ...
      <aside className={cn(
        ...
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold">لوحة الحاضنة</span>
          </div>
